      ...
      return (
        ...
        >
          <path
            ...
            fill='red'
            className='cursor-pointer hover:fill-pink-600'
          />
        </svg>
      );
    case "home":
...
export function bookMarkIcon(item: movieType, path: string) {
  const {
    itemsToShow,
    setTrendingItems,
    trendingItems,
  ...
  const toggleMarkIcon = (id: number) => {
    if (path === "/home") {
      setTrendingItems(
        trendingItems.map((item: movieType) =>
          item.id === id
    ...
    setItemsToShows(
      itemsToShow.map((item: movieType) =>
        item.id === id
          ? { ...item, bookMarked: !item.bookMarked }
          : item
      )
